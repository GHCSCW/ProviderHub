...
namespace ProviderHubService
{
    public class DataLayer : IDisposable
    {
        ...
        public dynamic GetMapResult(dynamic idResult, IEnumerable<dynamic> records) {
            ...
            var test_num_rows = 100; var runFullJob = true; //only used to test subset (10 or any x < # import file rows) rows for validating basic syntax...
            //START TRANSACTION
            ...
            /*IF YOU WANT TO USE SEED_IDs FROM DB -- caused bad errors for me! so i just use OUTPUT Inserted.ID instead
            if (ds.Tables[0].Rows.Count > 0)
            {
            ...
            if (runFullJob) { 
                ...
                if (staging_row.npi_match)
                {
                    ...
                    }catch(Exception e) { /*MI and/or gender not specified*/ }
                ...
                { //include !npi_match for clarity only; not needed
                    ...
                    _gender = (_dict["Gender"] as string == "Female") ? 1 : (_dict["Gender"] as string == "Male") ? 2 : 3;
                        ...
                        //If _dict["NPI"] string length > 10, mark as invalid/fatfingered NPI but just truncate, need to mark and look into later...
                ...
                if (valid_match)
                {
                    ...
                    string termcheckP="",termcheckF="",termcheckS="";
                        termcheckP = _pid.ToString(); termcheckF = Convert.ToInt32(((IDictionary<string, string>)staging_row.IDd_fac.matchedFac)["FACILITY_ID"]).ToString();
                        termcheckS = Convert.ToInt32(((IDictionary<string, string>)staging_row.IDd_spec.matchedSpec)["SPECIALTY_ID"]).ToString();
                    ...
                    try
                    {
                        foreach (dynamic additional_row in staging_row.provider_row.additional_matched_rows)
                        {
                            ...
                            termcheckP += "," + _pid; termcheckF += "," + Convert.ToInt32(((IDictionary<string, string>)additional_row.IDd_fac.matchedFac)["FACILITY_ID"]).ToString();
                            termcheckS += "," + Convert.ToInt32(((IDictionary<string, string>)additional_row.IDd_spec.matchedSpec)["SPECIALTY_ID"]).ToString();
                    ...
                    catch (Exception e) { } /*additional_row error*/
                    ...
                    try {
                        sql = "providerhub.dbo.sp_TermVendorRelationshipsNotFound";
                        sqlParams = new SqlParameter[] {
                            new SqlParameter("@PROVIDER_ID", SqlDbType.Int) { Value = _pid },
                            new SqlParameter("@VENDOR_ID", SqlDbType.Int){ Value = 48 },
                            new SqlParameter("@PROVIDER_IN", SqlDbType.VarChar) { Value = termcheckP },
                            new SqlParameter("@FACILITY_IN", SqlDbType.VarChar) { Value = termcheckF },
                            new SqlParameter("@SPECIALTY_IN", SqlDbType.VarChar) { Value = termcheckS }
                        };
                        ds = dataLayer.ExecuteDataSet(sql, CommandType.StoredProcedure, 0, sqlParams);
                    }
                    catch (Exception e) {
        ...
        public dynamic GetTermResult(dynamic mapResult, dynamic idResult, IEnumerable<dynamic> records) {
            ...
            {
                //CURRENTLY DOING IN THE LAST SP OF MAP RESULT SINCE ITS TOO SHORT TO NEED IT'S OWN FUNCTION -- FEEL FREE TO MOVE "TermVendorRelationships..." SP HERE THOUGH
                // IT would slow down the job by adding another loop through every record, though...keep that in mind.
